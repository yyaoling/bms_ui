                    ...
                    <Col>
                      ...
                      >
                        <!-- <Date-picker
                          type="date"
                          placeholder="选择日期"
                          v-model="formBook.publishDateTime"
                        ></Date-picker> -->
                        <el-date-picker
                          v-model="formBook.publishDateTime"
                          type="month"
              ...
              <Row type="flex" :gutter="30" justify="center">
                <Col :span="23">
                  <FormItem label="图书简介" prop="introduction">
                    <Simditor
                      v-model="formBook.introduction"
                      placeholder="图书简介"
                    ></Simditor>
                  </FormItem>
                  <FormItem label="作者简介" prop="authorIntroduction">
                    <Simditor
                      v-model="formBook.authorIntroduction"
                      placeholder="作者简介"
                    ></Simditor>
                  </FormItem>
                  <FormItem>
...
import { uploadCover, checkBookExists, addBook } from "@/pages/admin/api";
...
export default {
  ...
  data() {
    ...
    const checkStockValue = async (rule, value, callback) => {
      if (
        this.formBook.stock === null ||
        this.formBook.stock === "" ||
        this.formBook.stock < 0
      ) {
    ...
    const checkTags = async (rule, value, callback) => {
      if (this.formBook.tags.length != 0) callback();
      else callback(new Error("请至少输入一个标签"));
    };
    return {
      inputVisible: false,
  ...
  methods: {
    ...
    addBook() {
      ...
      this.$refs["formBook"].validate(async (valid) => {
        if (valid && this.formBook.coverImage) {
          await checkBookExists(this.formBook.isbn).then(async (recCheck) => {
            if (recCheck.data === true) {
              this.$Message.error("该图书已存在");
            } else {
              this.loadingSaveBtn = true;
              // let byteString = null,
              //   baseData = this.uploadImgSrc;
              // if (baseData.split(",")[0].indexOf("base64") >= 0) {
              //   byteString = atob(baseData.split(",")[1]);
              // } else {
              //   byteString = unescape(baseData.split(",")[1]);
              // }
              // let mimeString = baseData
              //   .split(",")[0]
              //   .split(":")[1]
              //   .split(";")[0];
              ...
              // this.formBook.coverImage =
              //   "https://bms-1302010632.cos.ap-nanjing.myqcloud.com/cover/" +
              //   fileName;
              this.formBook.publishDateTime = dayjs(
                this.formBook.publishDateTime
              ).format("YYYY-MM");
              let addData = Object.assign({}, this.formBook);
              await addBook(addData).then((resAdd) => {
                /// console.log(resAdd);
                if (resAdd.status === 201) {
                  this.chanageVisible(), this.$Message.success("创建成功");
