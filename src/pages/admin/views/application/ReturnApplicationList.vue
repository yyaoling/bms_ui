...
export default {
  ...
  methods: {
    ...
    async acceptApplication(application) {
      await acceptReturnApplication(application.aid).then(async resAccept => {
        if (resAccept.status === 204) {
          let resReturn = await returnRecord(application.rid, application.score, application.comment);
          if (resReturn.status === 204) {
    ...
    async rejectApplication(aid) {
      let resReject = await rejectReturnApplication(aid);
