...
import {
  getBorrowApplicationCount,
  getBorrowApplicationList,
  deleteBorrowApplication,
  acceptBorrowApplication,
...
export default {
  ...
  methods: {
    async getApplicationList() {
      ...
      let resBorrowApplicationCount = await getBorrowApplicationCount(
          this.query.keyWord, this.query.userName, this.query.state);
      let resBorrowApplicationList = await getBorrowApplicationList(
          this.query.keyWord, this.query.userName, this.query.state, this.query.page, this.query.limit
      );
      this.total = resBorrowApplicationCount.data;
      this.applications = resBorrowApplicationList.data;
      this.loadingTable = false;
    },
    async deleteApplication(aid) {
      let resDelete = await deleteBorrowApplication(aid);
    ...
    async acceptApplication(application) {
      await acceptBorrowApplication(application.aid).then(async resAccept => {
        if (resAccept.status === 204) {
          let resAdd = await addRecord({
            userName: application.userName,
            isbn: application.isbn,
            borrowDateTime: application.borrowDateTime,
            expectDateTime: application.expectDateTime,
          });
    ...
    async rejectApplication(aid) {
      let resReject = await rejectBorrowApplication(aid);
